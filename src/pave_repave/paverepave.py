...
def main():
    ...
    try:
        peer_node = Node(port=args.port_peer, token=args.token_cluster, ip=args.ip_peer)
        hsa_node = Node(port=args.port_hsa, token=args.token_cluster, ip=args.ip_hsa)
        spare_node = Node(port=args.port_spare, token=args.token_spare, ip=args.ip_spare)

        paverepave(peer=peer_node, hsa=hsa_node, spare=spare_node)
