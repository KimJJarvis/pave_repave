...
def leave_cluster(node_cluster: Node, node_peer: Node, integration_token: str) -> None:
    """
    ...
    # Log parameters
    logger.debug(
        f"leave-cluster called on cluster node {node_cluster}, peer node: {node_peer}, "
        f"integration_token provided: {bool(integration_token)}"
    )
    
    host = config.host if config.port_forward else node_cluster.ip
    base_url = f"https://{host}:{node_cluster.port}"
    ...
    response = make_single_api_request(url=url, bearer_token=node_cluster.token, method="POST", data=data)
    ...
    if http_status != 200:
        ...
        raise RuntimeError(f"leave_cluster returned unexpected HTTP status {http_status}: {response}")
    ...
    if "successfully initiated" not in status_msg.lower():
        ...
        raise RuntimeError(f"leave_cluster did not return expected success message. Got: {status_msg}")
