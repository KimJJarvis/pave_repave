...
from cluster_client.peer_info import get_id, peer_info
...
def repaveswitch_become_hsa(peer: Node, hsa: Node, spare: Node) -> None:
    ...
    become_hsa(spare=spare, ip_peer=hsa.ip, integration_token=integration_token)
...
def repave_switch_primary_secondary(peer: Node, hsa: Node, spare: Node) -> None:
    ...
    spare.token = hsa.token
    logger.debug("Calling switch_primary_secondary on HSA...")
    switch_primary_secondary(peer=hsa)
    logger.debug("✓ switch_primary_secondary initiated successfully")
...
def repave(peer: Node, hsa: Node, spare: Node, force: bool = True) -> None:
    """
    ...
    Args:
        ...
        hsa: HSA node
        spare: Spare node
        force: Whether to force leave cluster operations (default: True)
    ...
    funcs = [
        ...
        lambda p, h, sp: pave_leave_cluster_hsa(p, h, sp, force),
        repave_become_hsa,
    ]
...
def repaveswitch(peer: Node, hsa: Node, spare: Node, force: bool = True) -> None:
    """
