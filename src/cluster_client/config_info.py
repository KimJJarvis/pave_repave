...
def show_default_config() -> str:
    ...
    lines = []
    lines.append(f"| {'Option'.ljust(max_option)} | {'Type'.ljust(max_type)} | {'Default Value'.ljust(max_default)} |")
    lines.append(f"|{'-' * (max_option + 2)}|{'-' * (max_type + 2)}|{'-' * (max_default + 2)}|")
    
    for option, type_name, default in rows:
        lines.append(f"| {option.ljust(max_option)} | {type_name.ljust(max_type)} | {default.ljust(max_default)} |")
