...
def main():
    ...
    if spare_status.active_appliance != 1:
        print(
            f"[ERROR] Spare active_appliance is {spare_status.active_appliance}, expected 1",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if spare_status.id != hsa_status.id:
        print(
            f"[ERROR] Spare ID ({spare_status.id}) != HSA ID ({hsa_status.id})",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    print(
        f"  - Spare ({spare.ip}) has replaced the peer in the cluster", file=sys.stderr
    )
